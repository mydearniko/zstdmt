...
LIZARDMT_DCtx *LIZARDMT_createDCtx(int threads, int inputsize)
{
	...
	/* allocate ctx */
	ctx = (LIZARDMT_DCtx *) calloc(1, sizeof(LIZARDMT_DCtx));
	if (!ctx)
		return 0;
	...
	for (t = 0; t < threads; t++) {
		...
		/* setup thread work */
		if (LizardF_isError(LizardF_createDecompressionContext(&w->dctx,
							      LIZARDF_VERSION)))
			goto err_cwork;
	}
...
static void reset_writelists(LIZARDMT_DCtx *ctx)
{
